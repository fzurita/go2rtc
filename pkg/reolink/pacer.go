...
// schedule slot (e.g. one AAC AU set or one video access unit). write is the
// closure that actually delivers each packet to the matching receivers.
...
// next-emission cursor and wall-clock-relative scheduling. This is the port of
// reolinkproxy's mediaPacer, which is what keeps audio and video aligned: the
// camera delivers video in bursts, so writing packets out immediately makes the
// steady, self-clocked audio race ahead of the lagging video.
